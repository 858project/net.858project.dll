    ...
    public sealed class ByteAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            else if (value != null && (value is Byte))
            {
                ...
                //overime rozsah
                if (this.MinValue <= ((Byte)value) && this.MaxValue >= ((Byte)value))
                {
                    return ValidationResult.Success;
                }
            }
            return new ValidationResult(String.Format("Byte '{0}' and value '{1}' is not valid! [{2} - {3}]", validationContext.DisplayName, (value == null ? "NULL" : value.ToString()), this.MinValue, this.MaxValue));
        }
        #endregion
    }
}
